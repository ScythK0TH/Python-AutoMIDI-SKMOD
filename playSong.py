...
def processFile():
    global playback_speed
    global selectedsong
    with open(selectedsong, "r") as macro_file:
...
def get_song_choice():
    ...
    if not os.path.exists(song_folder):
        print("No song files detected.")
        return None
    
    songList = [f for f in os.listdir(song_folder) if f.lower().endswith('.txt')]
    if not songList:
        print("No song files detected.")
        return None

    print("\nType the number of a song file and press enter:\n")
    for i, song_file in enumerate(songList):
        print(f"{i + 1}: {song_file}")

    try:
        choice = int(input("> "))
        return os.path.join(song_folder, songList[choice - 1])
    except (IndexError, ValueError):
        print("Invalid selection. Please try again.")
        return None

def main():
    global isPlaying, infoTuple, playback_speed, selectedsong
